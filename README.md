...
Each detected press prints live (`Press N: peak Fz = … at TCP=[…]`) and also updates a **live 3D view** of the TCP path over the cone surface, written to `pyForceDAQ/cone_data/live_view.html`. Leave the tab open; it auto-refreshes every `LIVE_PLOT_REFRESH_S` (default `1` s) and shows the cone surface (gray), the TCP path so far (black), and a red diamond + label (`#N <peak Fz>N`) at each detected press.

If you're recording on the same machine you're sitting at, it opens automatically in your default browser as soon as recording starts (it checks `DISPLAY`/`WAYLAND_DISPLAY` first, so it never tries on a headless box). **If you're recording on the remote DAQ PC but watching from your workstation** — `webbrowser.open()` only ever opens a browser on the machine running the script, which doesn't help here — instead open the file through the DAQ PC's existing sshfs mount (the same one `sync_cone_data.sh` uses). Every run prints the exact path to copy-paste on the viewing machine, e.g.:

```
Viewing from another machine? Open this path there:
  ~/remote-server/pyForceDAQ/cone_data/live_view.html
```
...
| Live plot re-render / auto-refresh interval | `LIVE_PLOT_REFRESH_S = 1.0` s | `pyForceDAQ/record_cone_press.py` |
| Live plot trajectory decimation / window | `LIVE_PLOT_DECIMATE = 4`, `LIVE_PLOT_MAX_POINTS = 6000` | `pyForceDAQ/record_cone_press.py` |
